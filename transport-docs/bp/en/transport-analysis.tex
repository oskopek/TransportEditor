...
invalid states arising from inconsistent $in(p)$ and $at(p)$ state variable values,
...
$$(l+r)^v \cdot (f_{max})^v \cdot (l+v)^p,$$ where $r$ represents the number of roads
and $f_{max}$ the maximum fuel capacity for vehicles (this is a simplification where all vehicles
...
\TODO{not actually $f_{max}$, but the number of fuel vlaues it can take.. but it may be a good approximation... describe}

...
That does not mean domain knowledge makes Transport easy.
...
which we can do in polynomial time.
With fuel, there is no straightforward way \TODO{Later: Verify} of determining
if a plan exists --- however, we can verify that such a plan is valid in polynomial time, hence \textsc{Plan-Existence} is at least in NP after adding fuel.

We will now show that \textsc{Plan-Length} is NP-complete for Transport even with domain knowledge.
\begin{thm}
For all $L \in \N$, the problem of deciding whether a plan of length $L$
for sequential Transport without fuel exists (\textsc{Plan-Length}) with the usage of prior domain knowledge is NP-complete.
\end{thm}
\begin{proof}
It trivial to see that the problem is in NP: if we get a plan of length $\leq L$,
we can simulate the state changes done by the effects of individual actions and verify that the preconditions
of every action holds in the respective intermediate state. At the end, 
we verify that the goal condition holds in the final state.
All of this can be done in polynomial time. \TODO{Later: Probably $O(L)$?}

We will now show that we can reduce an NP-complete decision problem to our problem in polynomial time,
hence proving that all NP-complete problems are reducible to our problem,
and therefore, our problem is NP-hard.

According to the Cook-Levin theorem \citep{Cook1971}, the problem of determining whether a Boolean formula in Conjunctive Normal Form (CNF) is NP-complete.

We will now show that we can transform a Transport problem instance
to a CNF Boolean formula in polynomial time.
Using Figure~\ref{fig:classical-strips},
we generate $L$
...
is of equal length or longer: $p$ could have arrived at $B$ at the same time as $v$ but no sooner, and therefore the \pickup{} action could have taken place at either location and the plan would remain of the same length. \TODO{Later: Verify that this actually holds.}
...
\item \textbf{\TODO{Later: Add another one}}
\end{enumerate}

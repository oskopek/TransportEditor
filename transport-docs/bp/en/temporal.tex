...
\end{itemize}
Since we only schedule valid sequential plans,
it cannot happen that a package is scheduled to be concurrently loaded
into two different vehicles --- in such sequential plans,
pairs of \pickup{} and \drop{} actions for a single package
do not overlap each other. Therefore, a mutex relation will be constructed
between the \drop{} action of the first pair and the \pickup{} action of the second pair.
...
In case it is efficient to change the internals of sequential planners,
...
and $\Delta$ is set to $3 \cdot 10^{6}$ initially.

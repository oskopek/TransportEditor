...
\While{cancel not requested} \Comment{Cancelled by an external request}
...
$$\varphi_i = \frac{\exp((\mt{spd}(location(p), \mt{at}(v_i)) + 1)/T)}{\sum_{j = 1}^{|V|} \exp((\mt{spd}(location(p), \mt{at}(v_j)) + 1)/T)}.$$
...
In our experiments, we use a fixed $T = 0.05$, to prefer the nearest vehicles.
...
We will evaluate both the Nearby and Distribution
planners of the Randomized Restart Around Path

...
\TODO{proofread this and distinguish optimality vs. tree optimality properly}
...
\begin{align*}
h_4 = h_0' + \sum_{p \in P} \min\lbrace
...
\end{thm}
\begin{proof}
Let $s_0$ be the initial state and $s_g$ the nearest goal state.
Let $\pi$ be a plan from $s_0$ to $s_g$.

It holds that $h_4(s_g) = 0,$ because the minimum for each package is achieved at 0,
due to all packages being at their target locations.
Therefore, there exists a pair of successive states $s_i$ and $s_{i+1}$ in the intermediate states of $\pi$,
such that: $$h_4(s_i) > h^*(s_i) \And h_4(s_{i+1}) \leq h^*(s_{i+1}),$$
where $h^*$ is represents the optimum distance to the nearest goal state.
Let $s_j$ and $s_{j+1}$ be the last such states, with the action $a$
transitioning between them.
We will show that the value of $h_4$ 

If $a$ was a \pickup{} or \drop{} action,
only the $h'_0$ part of $h_4$ could have changed,
and $h'_0$ is trivially admissible.
Therefore, $a$ was a \drive{} action and the value of $h'_0$ does
not change on a \drive{} action.

The minimum value for a package in the driving vehicle
could not have changed at all, because it is at 0 and will stay so.

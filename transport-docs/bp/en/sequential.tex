...
Throughout this process, we will leverage the acquired
...
Since we generally do not know which action (state transition) to choose in a given state, the choice is usually delegated to a suitable
...
where $a \in A : \gamma(s, a) = s_{n},$ and for all goal states $s_g$, it holds that $h(s_g) = 0$.
...
The simplest domain-specific heuristic that is applicable to all variants of Transport, apart from the zero heuristic $h_0 \equiv 0$,
is one that counts the minimum number of \pickup{} and \drop{} actions
necessary to reach a goal state.

To obtain the correct count, we simply add 1 for each
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.36\textwidth]{../imga/heursit_h2}
...
In each iteration of the aforementioned algorithm,
...
over capacitated over the whole course

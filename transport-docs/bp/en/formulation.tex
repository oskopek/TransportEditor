...
\end{center}
\caption[Visualization of the \texttt{p13} problem of sequential Transport from IPC 2008.]{Road graph visualization of the \texttt{p13} problem of the seq-sat track of IPC 2008.
...
The numeric variant again adds the \refuel{} operator along with
...
On top of fuel-related predicates and functions from \verb+transport-numeric+, temporal Transport adds:
\begin{itemize}
...
\end{itemize}
Figure~\ref{code:statevar-temporal} shows the temporal state-variable representation of Transport temporal, using a shorter but clearer notation.
...
makes cooperation of vehicles practically hard to model, whereas in Transport this is one of the fundamental elements.

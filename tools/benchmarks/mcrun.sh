...
if [ -z "$tfd_flag" ]; then
qsub -m e -l cl_luna -l select=1:ncpus=6:mem=12gb:scratch_local=10gb -l walltime=1:00:00 -- "$HOME/git/$TEvariant/tools/benchmarks/mcbenchmark.sh" "configs/$config.json" &
else
qsub -m e -l cl_luna -l select=1:ncpus=1:mem=35gb:scratch_local=10gb -l walltime=1:15:00 -- "$HOME/git/$TEvariant/tools/benchmarks/mcbenchmark.sh" "configs/$config.json" &
fi

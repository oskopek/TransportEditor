...
public class RandomizedRestartWithOnPathPickupPlanner extends AbstractPlanner {
        ...
        while (true) {
            ...
            while (!current.isGoalState() && current.getTotalTime() < bestPlanScore) {
                ...
                while (true) {
                    ...
                    curCapacity = chosenVehicle.getCurCapacity().getCost();
                ...
                List<Action> newActions = findPlan(domain, current, chosenVehicle.getName(), chosenPackages, curCapacity);
    ...
    private List<Action> findPlan(Domain domain, ImmutablePlanState current, String chosenVehicleName,
            List<Package> chosenPackages, int capacityLeft) {

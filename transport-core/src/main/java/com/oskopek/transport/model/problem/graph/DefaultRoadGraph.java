...
/**
 * Simple RoadGraph implementation using {@link MultiGraph} as a base type. Represents an <strong>oriented</strong>
 * graph. Only a single oriented edge between two ordered nodes
 * (i.e. a total of two edges between a set of two nodes).
 */
public class DefaultRoadGraph extends MultiGraph implements RoadGraph {
     ...
     */
    private DefaultRoadGraph(RoadGraph graph) {
        this(graph.getId());
     ...
     */
    public DefaultRoadGraph(String id) {
        super(id);
    }

    @Override
    public DefaultRoadGraph copy() {
        return new DefaultRoadGraph(this);
    }
    ...
    public Stream<Road> getAllRoadsBetween(Location l1, Location l2) {
        ...
        return n1.getLeavingEdgeSet().stream().filter(edge -> edge.getNode1().equals(n2)).map(
                edge -> edge.getAttribute("road"));
    }
    ...
    @Override
    public int hashCode() {
        return new HashCodeBuilder(17, 37).append(getAllLocations().toArray()).append(getAllRoads().toArray())
                .toHashCode();
    }
    ...
    public boolean equals(Object o) {
        ...
        return new EqualsBuilder().append(getAllLocations().collect(Collectors.toSet()),
                that.getAllLocations().collect(Collectors.toSet())).append(getAllRoads().collect(Collectors.toSet()),
                that.getAllRoads().collect(Collectors.toSet())).isEquals();
    }
